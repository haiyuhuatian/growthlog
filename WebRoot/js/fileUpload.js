...
var uploadTools = {
    ...
    "getInitOption":function(uploadId){
        ...
        var initOption={
            "uploadId":uploadId,
            "uploadUrl":"${base}/member/diary!uploadImage.action",//必须，上传地址
    ...
    "initWithUpload":function(opt){
        ...
        $("#"+uploadId+" .uploadBts .uploadFileBt i").css("color","#f69fc4");
        $("#"+uploadId+" .uploadBts .uploadFileBt i").attr("title","上传")
    },
    /**
     * 初始化清除文件
     * @param opt
    ...
    "initWithCleanFile":function(opt){
        ...
        $("#"+uploadId+" .uploadBts .cleanFileBt i").css("color","#037f88");
        $("#"+uploadId+" .uploadBts .cleanFileBt i").attr("title","清除");
    ...
    "initWithLayout":function(opt){
        ...
        btsStr += "<div>";
        btsStr += "<div class='selectFileBt'>选择照片</div>";
        btsStr += "</div>";
    ...
    "initWithDeleteFile":function(opt){
        var uploadId = opt.uploadId;
        $("#"+uploadId+" .fileItem .status i").attr("id",inputNum-1);
        $("#"+uploadId+" .fileItem .status i").on("click",function(){
        	var myId =  $("#"+uploadId+" .fileItem .status i").attr("id");
        	$("#"+uploadId+"_file"+myId).remove();
    ...
    "addFileList":function(fileList,opt){
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(uploadTools.fileIsExit(fileList[i],opt)){
                ...
                inputNum = inputNum-1;
                $("#"+uploadId+"_file"+inputNum).remove();
                continue;
            }
...
var uploadEvent = {
    ...
    "selectFileEvent":function(opt){
        ...
        inputObj.setAttribute('id',uploadId+'_file'+inputNum);
        inputNum = inputNum+1;
