	...
	<head>
		...
		<script type="text/javascript">
			XBack.listen(function(){
		 		window.location = "${base}/mine/member!detailForCircle.action?user.id="
		 						+$("#member").val()
		 						+"&circle.id="
		 				        +$("#circleId").val();
			});
				...
				<ul id="ulId">
					<c:forEach items="${diaries }" var="d" varStatus="status">
						<li class="${(status.index+1)%2==0?'rightLi':'leftLi' }" 
							onclick="window.location='${base}/mine/circle!diaryDetail.action?circle.id=${circle.id }&diary.id=${d.id }'">
		 ...
		 <script type="text/javascript">
				...
				function loadData(){
					var	lastIdSign = $("#lastIdSign").val();
					 $.ajax({ 
						 type: "post", 
						 url: "circle!loadPersonDiaryData.action", 
						 async: false,
						 dataType: "json", 
						 data:{"lastIdSign":lastIdSign,"circle.id":circleId,"member":member},
				...
				function Refresh() {
					window.location = '${base}/mine/circle!personDiaryList.action?circle.id='+circleId+"&member="+member;
				}
						...
						if(loadDatas != ''){
							...
							$.each(loadDatas,function(i,item){
								...
								li.innerHTML = '<figure style="display:block;float:left;margin:0 5px 5px 0;width:100%;">'
									 ...
									 + '</div>';
								li.onclick=function(){
									window.location='${base}/mine/circle!diaryDetail.action?circle.id='+circleId+'&diary.id='+item.id;
								};
