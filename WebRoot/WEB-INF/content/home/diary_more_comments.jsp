	...
	<head>
		...
		<script type="text/javascript" >
			 	XBack.listen(function(){
				  window.location = "${base}/home/diary!detail.action?diary.id="
						  +$("#diaryId").val()
						  +"&model="
						  +$("#model").val();
				});
	...
	<body style="background-color:#F0EFEE;">
				...
				<c:forEach items="${commentList }" var="c">
					<li id="comment${c.id }">
						...
						<div class="comment-div">
							...
							<p class="show-family" 
								onclick="window.location='${base}/home/diary!showSonComments.action?diaryComment.id=${c.id}&model=${model }'">
		...
		<script type="text/javascript">
				...
				function Refresh() {
					...
					window.location = '${base}/home/diary!moreComments.action?&diary.id='+diaryId+'&model='+model;
				}
							...
							$.each(loadDatas,function(i,item){
								...
								if(item.like_id == ""){
									...
									if(item.family_num != "0"){
										inHTML = inHTML 
											   + '<p class="show-family"'
											   + 'onclick="window.location=\'${base}/home/diary!showSonComments.action?diaryComment.id='+item.id+'&model='+model+'\'">'
